...
const API_ORIGIN = 'https://afaia-maps-production.up.railway.app';
...
function getDeviceId() {
  let deviceId = localStorage.getItem('afaia_device_id');
  if (!deviceId) {
    deviceId = `device_${crypto.randomUUID()}`;
    localStorage.setItem('afaia_device_id', deviceId);
  }
  return deviceId;
}
...
async function tryRefreshToken(refreshToken) {
  ...
  try {
    ...
    if (data.refreshToken) {
      localStorage.setItem('afaia_refresh_token', data.refreshToken);
    }
...
window.TracksAPI = {
  ...
  list(params = {}) {
    return apiFetch(`/tracks?${new URLSearchParams(params)}`);
  },
  ...
  addPoints(id, points) {
    return apiFetch(`/tracks/${id}/points`, {
      method: 'POST',
  ...
  finish(id) {
    return apiFetch(`/tracks/${id}/finish`, {
      method: 'POST'
    });
  },

  geojson(id) {
    return apiFetch(`/tracks/${id}/geojson`);
  },
...
window.WaypointsAPI = {
  ...
  list(params = {}) {
    return apiFetch(`/waypoints?${new URLSearchParams(params)}`);
  },

  get(id) {
    return apiFetch(`/waypoints/${id}`);
  },
  ...
  bbox(minLng, minLat, maxLng, maxLat) {
    ...
    return apiFetch(
      `/waypoints/bbox?minLng=${minLng}&minLat=${minLat}&maxLng=${maxLng}&maxLat=${maxLat}`
    );
...
window.MapsAPI = {
  ...
  list(params = {}) {
    return apiFetch(`/maps?${new URLSearchParams(params)}`);
  },

  get(id) {
    return apiFetch(`/maps/${id}`);
  },
  ...
  status(id) {
    return apiFetch(`/maps/${id}/status`);
  },
...
window.SyncAPI = {
  ...
  pull(since) {
    return apiFetch(`/sync/pull${since ? `?since=${since}` : ''}`);
  },
...
window.PhotosAPI = {
  ...
  list(params = {}) {
    return apiFetch(`/photos?${new URLSearchParams(params)}`);
  },
